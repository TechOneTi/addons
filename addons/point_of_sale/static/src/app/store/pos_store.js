...
export class PosStore extends Reactive {
    ...
    async _syncAllOrdersFromServer() {
        ...
        let ordersJson = await this._getOrdersJson();
        
        await this._getMissingProducts(ordersJson);
        
        let oldOrders = [...this.get_order_list()];
      
        this._replaceOrders(oldOrders, ordersJson);
        
        let addedOrder = null;
        
        if (ordersJson.length > oldOrders.length) {
            addedOrder = ordersJson.find(newOrder => !oldOrders.some(oldOrder => oldOrder.id === newOrder.id));       
        }
