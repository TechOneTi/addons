    ...
    async setup(env, { popup, orm, number_buffer, hardware_proxy, barcode_reader, ui }) {
        ...
        setInterval(async () => {
            if(this.selectedOrder){
                const currentOrderName = this.selectedOrder.name;
                const ordersJson = await this._getOrdersJson();
                const orderToPrinting = ordersJson.find(item => item.name === currentOrderName);
                console.log("STR ORDER PRINTING",orderToPrinting);
                const existingOrderForPrintingStorage =  window.localStorage.getItem('order_for_printing');
                const existingOrderForPrinting = existingOrderForPrintingStorage ?  JSON.parse(existingOrderForPrintingStorage) : null;
                const existingOrderName = existingOrderForPrinting ? existingOrderForPrinting.name : null;
                if(orderToPrinting && existingOrderName !== currentOrderName){
                    window.localStorage.setItem('order_for_printing', JSON.stringify(orderToPrinting));
                }
            }
        }, 1000);
        //this.order_for_printing = await this._getOrdersJson();
    ...
    getReceiptHeaderData(order) {
        const orderForPrinting = window.localStorage.getItem('order_for_printing');
        let parsedOrder = null;
        if (orderForPrinting) {
            try {
                parsedOrder = JSON.parse(orderForPrinting);
            } catch (error) {
                console.error('Error parsing order for printing:', error);
            }
        }
        return {
